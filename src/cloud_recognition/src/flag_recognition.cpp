...
private:
    ...
    // 新增：第一组识别区域参数
    double det_roi1_min_x_, det_roi1_max_x_;
    double det_roi1_min_y_, det_roi1_max_y_;
    double det_roi1_min_z_, det_roi1_max_z_;
    // 新增：第二组识别区域参数
    double det_roi2_min_x_, det_roi2_max_x_;
    double det_roi2_min_y_, det_roi2_max_y_;
    double det_roi2_min_z_, det_roi2_max_z_;
...
public:
    FlagDetector() : nh_(), private_nh_("~")
    {
        ...
        // 新增：读取第一组识别区域参数
        private_nh_.param("det_roi1_min_x", det_roi1_min_x_, 0.0);
        private_nh_.param("det_roi1_max_x", det_roi1_max_x_, 8.0);
        private_nh_.param("det_roi1_min_y", det_roi1_min_y_, -8.0);
        private_nh_.param("det_roi1_max_y", det_roi1_max_y_, 0.0);
        private_nh_.param("det_roi1_min_z", det_roi1_min_z_, 0.2);
        private_nh_.param("det_roi1_max_z", det_roi1_max_z_, 3.5);

        // 新增：读取第二组识别区域参数
        private_nh_.param("det_roi2_min_x", det_roi2_min_x_, 0.0);
        private_nh_.param("det_roi2_max_x", det_roi2_max_x_, 8.0);
        private_nh_.param("det_roi2_min_y", det_roi2_min_y_, -8.0);
        private_nh_.param("det_roi2_max_y", det_roi2_max_y_, 0.0);
        private_nh_.param("det_roi2_min_z", det_roi2_min_z_, 0.2);
        private_nh_.param("det_roi2_max_z", det_roi2_max_z_, 3.5);
        ...
        ROS_INFO("Detection Region 1: [%.2f, %.2f] x [%.2f, %.2f] x [%.2f, %.2f]",
                 det_roi1_min_x_, det_roi1_max_x_, det_roi1_min_y_, det_roi1_max_y_, det_roi1_min_z_, det_roi1_max_z_);
        ROS_INFO("Detection Region 2: [%.2f, %.2f] x [%.2f, %.2f] x [%.2f, %.2f]",
                 det_roi2_min_x_, det_roi2_max_x_, det_roi2_min_y_, det_roi2_max_y_, det_roi2_min_z_, det_roi2_max_z_);
    ...
    void cloudCallback(const sensor_msgs::PointCloud2ConstPtr &msg)
    {
        ...
        if (roi_cloud->empty())
        {
            ...
            publishClusteredCloud(std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr>(), msg->header);
            return;
        }
        ...
        // 3. 聚类
        std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> all_clusters = clusterPointCloud(downsampled_cloud);

        // 4. 筛选：只保留完全位于 识别区域并集 内的聚类
        std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> valid_clusters;
        for (const auto &cluster : all_clusters)
        {
            if (isClusterFullyInDetectionUnionROI(cluster))
            {
                valid_clusters.push_back(cluster);
            }
        }

        ROS_INFO_THROTTLE(1.0, "Clusters fully in Detection Union ROI: %zu / %zu", valid_clusters.size(), all_clusters.size());

        // 发布聚类后的点云（用于调试，仅发布有效聚类）
        publishClusteredCloud(valid_clusters, msg->header);

        // 5. 检测刀旗（仅基于完全在 识别区域并集 内的聚类）
        std::vector<FlagObject> flags = detectFlags(valid_clusters);
...
private:
    ...
    std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> clusterPointCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud)
    {
        ...
        ROS_INFO_THROTTLE(1.0, "Found %zu raw clusters", clusters.size());
        return clusters;
    }

    // 新增：检查聚类是否完全在两个识别区域的并集内
    bool isClusterFullyInDetectionUnionROI(const pcl::PointCloud<pcl::PointXYZ>::Ptr &cluster)
    {
        if (cluster->empty())
            return false;
        ...
        {
            // 检查是否在ROI1外
            if (point.x < det_roi1_min_x_ || point.x > det_roi1_max_x_ ||
                point.y < det_roi1_min_y_ || point.y > det_roi1_max_y_ ||
                point.z < det_roi1_min_z_ || point.z > det_roi1_max_z_)
            {
                in_roi1 = false; // 不在ROI1内
            }

            // 检查是否在ROI2外
            if (point.x < det_roi2_min_x_ || point.x > det_roi2_max_x_ ||
                point.y < det_roi2_min_y_ || point.y > det_roi2_max_y_ ||
                point.z < det_roi2_min_z_ || point.z > det_roi2_max_z_)
            {
                in_roi2 = false; // 不在ROI2内
            }

            // 如果既不在ROI1也不在ROI2，则直接返回false
            if (!in_roi1 && !in_roi2)
            {
                return false;
            }
                               ...
                               const std_msgs::Header &header)
    {
        ...
        {
            // 发布一个空的点云消息，以清空RViz中的显示
            sensor_msgs::PointCloud2 empty_msg;
            empty_msg.header = header;
            empty_msg.width = 0;
            empty_msg.height = 1;
            empty_msg.is_dense = true;
            empty_msg.point_step = 16; // 假设PointXYZRGB是16字节
            empty_msg.row_step = 0;
            clustered_cloud_pub_.publish(empty_msg);
        }
    }
